...
import { UserAccount, Subscription } from '../types';
...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const dbKey = `user_db_${userId}`;
    const storedUserRaw = localStorage.getItem(dbKey);
    ...
    if (isLogin) {
      ...
      onLogin(storedUser as UserAccount);
    } else {
      if (storedUserRaw) {
      ...
      onLogin(newUser as UserAccount);
    }
  };
    ...
    <div className="min-h-[85vh] flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md bg-white p-10 rounded-[2.5rem] shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-500 relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 relative">
          ...
          <button 
            type="submit"
            className="w-full bg-brand-600 hover:bg-brand-700 text-white font-black py-5 rounded-2xl shadow-xl shadow-brand-100 transition-all uppercase tracking-[0.15em] text-xs mt-4 active:scale-[0.98]"
          >
        ...
        <div className="mt-10 text-center relative border-t border-slate-100 pt-6">
          <button 
            ...
            className="text-[11px] font-black text-slate-500 uppercase tracking-widest hover:text-brand-600 transition-colors"
          >
            {isLogin ? (
