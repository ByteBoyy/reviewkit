...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const trendData = useMemo(() => {
    ...
    const sorted = [...filteredReviews].sort((a, b) => {
      const dateA = new Date(a.date || a.review_date);
      const dateB = new Date(b.date || b.review_date);
      return dateA.getTime() - dateB.getTime();
    });
    
    sorted.forEach(r => {
      try {
        const d = new Date(r.date || r.review_date);
        if (isNaN(d.getTime())) {
          console.warn('Invalid date for review:', r);
          return;
        }
        ...
        console.error('Error processing review date:', e, r);
      }
    });
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {(['gmb', 'trustpilot', 'tripadvisor', 'getyourguide'] as const).map(source => { 
                const linkedAccounts = location.linkedAccounts || {};
                const sourceData = linkedAccounts[source as keyof typeof linkedAccounts];
                
                // ✅ FIXED: Properly check if source is connected by verifying lastSync exists
                const isConnected = sourceData && 
                                    typeof sourceData === 'object' && 
                                    'lastSync' in sourceData && 
                                    sourceData.lastSync !== null && 
                                    sourceData.lastSync !== undefined;
                
                const lastSync = isConnected && sourceData ? sourceData.lastSync : undefined;
                const importCount = isConnected && sourceData ? (sourceData.importCount || 0) : reviews.filter(r => r.source === source).length;
